 ...
 *
 * <p>TODO: These tests should be mostly retired. Much of the test data that was originally here was
 * removed when the RBBI rules were updated to match the Unicode boundary TRs, and the data was
 * found to be invalid. Much of the remaining data has been moved into the rbbitst.txt test data
 * file, which is common between ICU4C and ICU4J. The remaining test data should also be moved, or
 * simply retired if it is no longer interesting.
 */
...
public class RBBITest extends CoreTestFmwk {
    ...
    public void TestThaiDictionaryBreakIterator() {
        ...
        char ctext[] = {
            0x0041, 0x0020, 0x0E01, 0x0E32, 0x0E23, 0x0E17, 0x0E14, 0x0E25, 0x0E2D, 0x0E07, 0x0020,
            0x0041
        };
        ...
        if (((RuleBasedBreakIterator) brk).getRuleStatusVec(fillInArray) != 1
                || fillInArray[0] != 0) {
            errln(
    ...
    // TODO: Move these test cases to rbbitst.txt if they aren't there already, then remove this
    ...
    public void TestTailoredBreaks() {
                    ...
                    while (foundOffsetsCount > 0) {
                        ...
                        if (offset != foundOffsets[--foundOffsetsCount]) {
                            ...
                            String textToDisplay =
                                    (text.length() <= 16) ? text : text.substring(0, 16);
                            errln(
                                    "For type "
                                            + type
                                            + " "
        ...
        // KIND_SENTENCE "el"
        final String elSentText =
                "\u0391\u03B2, \u03B3\u03B4; \u0395 \u03B6\u03B7\u037E \u0398 \u03B9\u03BA. "
                        + "\u039B\u03BC \u03BD\u03BE! \u039F\u03C0, \u03A1\u03C2? \u03A3";
        final int[] elSentTOffsets = {8, 14, 20, 27, 35, 36};
        final int[] elSentROffsets = {20, 27, 35, 36};
        // KIND_CHARACTER "th"
        final String thCharText =
                "\u0E01\u0E23\u0E30\u0E17\u0E48\u0E2D\u0E21\u0E23\u0E08\u0E19\u0E32 "
                        + "(\u0E2A\u0E38\u0E0A\u0E32\u0E15\u0E34-\u0E08\u0E38\u0E11\u0E32\u0E21\u0E32\u0E28) "
                        + "\u0E40\u0E14\u0E47\u0E01\u0E21\u0E35\u0E1B\u0E31\u0E0D\u0E2B\u0E32 ";
        final int[] thCharTOffsets = {
            1, 2, 3, 5, 6, 7, 8, 9, 10, 11, 12, 13, 15, 16, 17, 19, 20, 22, 23, 24, 25, 26, 27, 28,
    ...
    public void TestEquals() {
        ...
        // TODO: Tests when "if (fRData != other.fRData && (fRData == null || other.fRData ==
        // null))" is true
    ...
    public void TestBug7547() {
        try {
            ...
            fail(
                    "TestBug7547: RuleBasedBreakIterator constructor failed to throw an exception with empty rules.");
    ...
    public void TestBug12873() {
        ...
        final AssertionError[] assertErr =
                new AssertionError[1]; // saves an error found from within a thread
            ...
            public void run() {
                try {
                    ...
                    for (int loop = 0; loop < 100; loop++) {
                        ...
                        for (int actualBreak = localBI.first();
                                actualBreak != BreakIterator.DONE;
    ...
    public void TestBreakAllChars() {
        ...
        for (int breakKind = BreakIterator.KIND_CHARACTER;
                breakKind <= BreakIterator.KIND_TITLE;
                ++breakKind) {
            RuleBasedBreakIterator bi =
                    (RuleBasedBreakIterator)
                            BreakIterator.getBreakInstance(ULocale.ENGLISH, breakKind);
            bi.setText(s);
            int lastb = -1;
    ...
    public void TestBug12519() {
        RuleBasedBreakIterator biEn =
                (RuleBasedBreakIterator) BreakIterator.getWordInstance(ULocale.ENGLISH);
        RuleBasedBreakIterator biFr =
                (RuleBasedBreakIterator) BreakIterator.getWordInstance(ULocale.FRANCE);
    ...
    static class T13512Thread extends Thread {
        ...
        private static final BreakIterator BREAK_ITERATOR_CACHE =
    ...
    public void TestTableRedundancies() {
        RuleBasedBreakIterator bi =
                (RuleBasedBreakIterator) BreakIterator.getLineInstance(Locale.ENGLISH);
        String rules = bi.toString();
        bi = new RuleBasedBreakIterator(rules);
        // Build a break iterator from source rules.
        // Want to check the rule builder in Java, not the pre-built rules that are imported from
        // ICU4C.
        ...
        for (int c1 = 1; c1 < numCharClasses; c1++) {
            int limit =
                    c1 < fwtbl.fDictCategoriesStart ? fwtbl.fDictCategoriesStart : numCharClasses;
            for (int c2 = c1 + 1; c2 < limit; c2++) {
                assertFalse(
                        String.format("Duplicate columns (%d, %d)", c1, c2),
                        columns.get(c1).equals(columns.get(c2)));
        ...
        for (int r1 = 0; r1 < fwtbl.fNumStates; r1++) {
            for (int r2 = r1 + 1; r2 < fwtbl.fNumStates; r2++) {
                assertFalse(
                        String.format("Duplicate states (%d, %d)", r1, r2),
    ...
    public void TestTableRebuild() {
        ...
        // break iterator types yields the same tables as are imported from ICU4C as part of the
        // default data.
        List<RuleBasedBreakIterator> breakIterators = new ArrayList<>();
        breakIterators.add(
                (RuleBasedBreakIterator) BreakIterator.getCharacterInstance(Locale.ENGLISH));
        breakIterators.add((RuleBasedBreakIterator) BreakIterator.getWordInstance(Locale.ENGLISH));
        breakIterators.add(
                (RuleBasedBreakIterator) BreakIterator.getSentenceInstance(Locale.ENGLISH));
        ...
        for (RuleBasedBreakIterator bi : breakIterators) {
            ...
            assertTrue(
                    "Forward Table", RBBIDataWrapper.equals(bi.fRData.fFTable, bi2.fRData.fFTable));
            assertTrue(
                    "Reverse Table", RBBIDataWrapper.equals(bi.fRData.fRTable, bi2.fRData.fRTable));
        }
    }

    // Helper function to test 8/16 bits of trie and 8/16 bits of state table.
    private void testTrieStateTable(
            int numChar, boolean expectUCPTrieValueWidthIn8Bits, boolean expectStateRowIn8Bits) {
        ...
        assertEquals(
                "Number of char classes mismatch numChar=" + numChar,
                numChar + 4,
                dw.fHeader.fCatCount);
        assertEquals(
                "Number of states in Forward Table mismatch numChar=" + numChar,
                numChar + 3,
                fwtbl.fNumStates);
        assertEquals(
                "Number of states in Reverse Table mismatch numChar=" + numChar,
                numChar + 3,
                rvtbl.fNumStates);
        assertEquals(
                "Trie width mismatch numChar=" + numChar,
                expectUCPTrieValueWidthIn8Bits,
                has8BitsTrie);
        assertEquals(
                "Bits of Forward State table mismatch numChar=" + numChar,
                expectStateRowIn8Bits,
                has8BitRowDataForwardTable);
        assertEquals(
                "Bits of Reverse State table mismatch numChar=" + numChar,
                expectStateRowIn8Bits,
                has8BitRowDataReverseTable);
    ...
    public void Test8BitsTrieWith8BitStateTable() {
        testTrieStateTable(
                251, true /* expectUCPTrieValueWidthIn8Bits */, true /* expectStateRowIn8Bits */);
    }

    @Test
    public void Test16BitsTrieWith8BitStateTable() {
        testTrieStateTable(
                252, false /* expectUCPTrieValueWidthIn8Bits */, true /* expectStateRowIn8Bits */);
    }

    @Test
    public void Test16BitsTrieWith16BitStateTable() {
        testTrieStateTable(
                253, false /* expectUCPTrieValueWidthIn8Bits */, false /* expectStateRowIn8Bits */);
    }
    ...
    /**
     * Test that both compact (8 bit) and full sized (16 bit) rbbi tables work, and that there are
     * no problems with rules at the size that transitions between the two.
     *
     * <p>A rule that matches a literal string, like 'abcdefghij', will require one state and one
     * character class per character in the string. So we can make a rule to tickle the boundaries
     * by using literal strings of various lengths.
     *
     * <p>For both the number of states and the number of character classes, the eight bit format
     * only has 7 bits available, allowing for 128 values. For both, a few values are reserved,
     * leaving 120 something available. This test runs the string over the range of 120 - 130, which
     * allows some margin for changes to the number of values reserved by the rule builder without
     * breaking the test.
     */
    @Test
    public void TestTable_8_16_Bits() {
        ...
        for (int ruleLen = startLength; ruleLen <= endLength; ruleLen += increment) {
            ...
            bi.setText(testStr);
            assertEquals(
                    "The first boundary found in testStr should be at the length of the rule string",
                    ruleLen,
                    bi.next());

            // Reverse iteration. Do a setText() first, to flush the break iterator's internal cache
            // of previously detected boundaries, thus forcing the engine to run the safe reverse
            // rules.
            bi.setText(testStr);
    ...
    public void TestBug13590() {
        ...
        for (int ruleNum = 0; ruleNum < NUM_LOOKAHEAD_RULES; ++ruleNum) {
            ...
            rules.append('\'')
                    .append(firstChar)
                    .append((char) (firstChar + 1))
                    .append('\'')
                    .append(' ')
                    .append('/')
                    .append(' ')
                    .append('\'')
                    .append((char) (firstChar + 2))
                    .append((char) (firstChar + 4))
                    .append('\'')
                    .append(';')
                    .append('\n');
        }
        ...
        for (char c = (char) (STARTING_CHAR - 200);
                c < STARTING_CHAR + NUM_LOOKAHEAD_RULES * 4;
                ++c) {
            testString.append(c);
        }
    ...
    public void TestUnpairedSurrogate() {
        ...
        try {
            ...
            fail(
                    "TestUnpairedSurrogate: RuleBasedBreakIterator() failed to throw an exception with unpaired low surrogate.");
        } catch (IllegalArgumentException e) {
            // expected exception with unpaired surrogate.
        } catch (Exception e) {
            fail(
                    "TestUnpairedSurrogate: Unexpected exception while new RuleBasedBreakIterator() with unpaired low surrogate: "
                            + e);
        }
        ...
        try {
            new RuleBasedBreakIterator("a\ude00b;");
            fail(
                    "TestUnpairedSurrogate: RuleBasedBreakIterator() failed to throw an exception with unpaired high surrogate.");
        } catch (IllegalArgumentException e) {
            // expected exception with unpaired surrogate.
        } catch (Exception e) {
            fail(
                    "TestUnpairedSurrogate: Unexpected exception while new RuleBasedBreakIterator() with unpaired high surrogate: "
                            + e);
        }
    ...
    public void TestBug22579() {
        try {
            ...
            fail(
                    "TestBug22579: RuleBasedBreakIterator() failed to throw an exception with only string in an Unicode set.");
        } catch (IllegalArgumentException e) {
            // expected exception with only string inside an Unicode set.
        } catch (Exception e) {
            fail(
                    "TestBug22579: Unexpected exception while new RuleBasedBreakIterator() with only string in an Unicode Set: "
                            + e);
        }
    }
    ...
    public void TestBug22585() {
        try {
            ...
            fail(
                    "TestBug22585: RuleBasedBreakIterator() failed to throw an exception with unpaired high surrogate.");
        } catch (IllegalArgumentException e) {
            // expected exception with unpaired surrogate.
        } catch (Exception e) {
            fail(
                    "TestBug22585: Unexpected exception while new RuleBasedBreakIterator() with unpaired high surrogate: "
                            + e);
        }
        ...
        try {
            new RuleBasedBreakIterator("$a=[\ud94e];");
            fail(
                    "TestBug22585: RuleBasedBreakIterator() failed to throw an exception with unpaired low surrogate.");
        } catch (IllegalArgumentException e) {
            // expected exception with unpaired surrogate.
        } catch (Exception e) {
            fail(
                    "TestBug22585: Unexpected exception while new RuleBasedBreakIterator() with unpaired low surrogate: "
                            + e);
        }
    }
    ...
    public void TestBug22602() {
        try {
            ...
            RuleBasedBreakIterator bi = new RuleBasedBreakIterator(rules);
            fail(
                    "TestBug22602: RuleBasedBreakIterator() failed to throw an exception with a long string followed by a ';'.");
        } catch (IllegalArgumentException e) {
            // expected exception with a long string followed by a ';'.
        } catch (StackOverflowError e) {
            fail(
                    "TestBug22602: Unexpected exception while new RuleBasedBreakIterator() with a long string followed by a ';': "
                            + e);
        }
    }
    ...
    public void TestBug22636() {
        try {
            ...
            fail(
                    "TestBug22636: new RuleBasedBreakIterator() with a large status value inside {}:  should throw IllegalArgumentException");
        } catch (IllegalArgumentException e) {
