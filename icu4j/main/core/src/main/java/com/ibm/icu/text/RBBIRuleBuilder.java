...
class RBBIRuleBuilder {
    ...
    //
    // Status {tag} values.   These structures are common to all of the rule sets (Forward, Reverse,
    // etc.).
    //
    Map<Set<Integer>, Integer> fStatusSets =
            new HashMap<>(); // Status value sets encountered so far.
    ...
    void flattenData(OutputStream os) throws IOException {
        ...
        header[RBBIDataWrapper.DH_LENGTH] =
                totalSize; // fLength, the total size of all rule sections.
        ...
        header[RBBIDataWrapper.DH_RTABLE] =
                header[RBBIDataWrapper.DH_FTABLE] + forwardTableSize; // fRTable
        header[RBBIDataWrapper.DH_RTABLELEN] = reverseTableSize; // fRTableLen

        header[RBBIDataWrapper.DH_TRIE] =
                header[RBBIDataWrapper.DH_RTABLE] + header[RBBIDataWrapper.DH_RTABLELEN]; // fTrie
        header[RBBIDataWrapper.DH_TRIELEN] = fSetBuilder.getTrieSize(); // fTrieLen
        header[RBBIDataWrapper.DH_STATUSTABLE] = header[RBBIDataWrapper.DH_TRIE] + trieSize;
        header[RBBIDataWrapper.DH_STATUSTABLELEN] = statusTableSize; // fStatusTableLen
        header[RBBIDataWrapper.DH_RULESOURCE] =
                header[RBBIDataWrapper.DH_STATUSTABLE] + statusTableSize;
    ...
    void optimizeTables() {
        ...
        do {
            ...
            // Class 0 is unused and should not have other categories merged into
            // it. We allow merging categories *into* class 1 {eof} (but not the
            // reverse, so that eof remains in class 1).
            IntPair duplPair = new IntPair(1, 0);
