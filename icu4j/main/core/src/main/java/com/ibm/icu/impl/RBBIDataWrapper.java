 ...
 *
 * <p>This class provides access to the compiled break rule data, as it is stored in a .brk file.
 * Refer to the file common/rbbidata.h from ICU4C for further details.
 */
public final class RBBIDataWrapper {

    /**
     * A RBBI State Transition table, the form of the data used at run time in Java. These can be
     * created from stored ICU data, or built from rules. The structure corresponds closely to
     * struct RBBIStateTable in ICU4C.
     */
    public static class RBBIStateTable {
        ...
        /**
         * Char category number of the first dictionary char class, or the largest category number +
         * 1 if there are no dictionary categories.
         */
        public int fDictCategoriesStart;

        /**
         * Size of run-time array required for holding look-ahead results. Indexed by
         * row.fLookAhead.
         */
        ...
        /**
         * Length in bytes of the state table header, of all the int32 fields preceding fTable in
         * the serialized form.
         */
        ...
        static RBBIStateTable get(ByteBuffer bytes, int length) throws IOException {
            ...
            boolean use8Bits =
                    (This.fFlags & RBBIDataWrapper.RBBI_8BITS_ROWS)
    ...
    /**
     * Data Header. A struct-like class with the fields from the RBBI data file header. Not intended
     * for public use, declared public for testing purposes only.
     */
    ...
    /**
     * RBBI State Table Indexing Function. Given a state number, return the array index of the start
     * of the state table row for that state.
     */
    ...
    public static RBBIDataWrapper get(ByteBuffer bytes) throws IOException {
        ...
        if (This.fHeader.fMagic != 0xb1a0
                || !IS_ACCEPTABLE.isDataVersionAcceptable(This.fHeader.fFormatVersion)) {
        ...
        This.fRuleSource =
                new String(
                        ICUBinary.getBytes(bytes, This.fHeader.fRuleSourceLen, 0),
                        StandardCharsets.UTF_8);

        if (RuleBasedBreakIterator.fDebugEnv != null
                && RuleBasedBreakIterator.fDebugEnv.indexOf("data") >= 0) {
